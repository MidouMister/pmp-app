...
export const GanttSidebarItem: FC<GanttSidebarItemProps> = ({
  ...
  // Import the custom function at the top of your file
  const formatCustomFrenchDuration = (
    startDate: Date,
    endDate?: Date | null,
    includeTime: boolean = false
  ): string => {
    const tempEndDate =
      endDate && isSameDay(startDate, endDate) ? addDays(endDate, 1) : endDate;

    const actualEndDate = tempEndDate || new Date();
    const totalDays = differenceInDays(actualEndDate, startDate);
    ...
    if (months > 0) {
      result += `${months.toString().padStart(2, "0")}M`;
    }

    if (days > 0) {
      if (result) result += "-";
      result += `${days.toString().padStart(2, "0")}J`;
    }

    if (!endDate && includeTime) {
      result += " jusqu'à présent";
    }

    return result || "0 jour";
  };

  // Then in the GanttSidebarItem component, replace the duration calculation:
  const duration = formatCustomFrenchDuration(
    feature.startAt,
    feature.endAt,
    !feature.endAt
  );
...
export const GanttSidebarHeader: FC = () => (
  ...
  >
    ...
    <p className="flex-1 truncate text-left">Phases</p>
...
> = memo(({ label, date, id, onRemove, className }) => {
  ...
  return (
    ...
    >
      ...
      <div className={cn("h-full w-[2px] bg-card", className)} />
    </div>
  );
});
...
export const GanttToday: FC<GanttTodayProps> = ({ className }) => {
  ...
  return (
    ...
    >
      <div
        className={cn(
          "group pointer-events-auto sticky top-0 flex select-auto flex-col flex-nowrap items-center justify-center whitespace-nowrap rounded-b-md bg-background px-2 py-1 text-foreground text-xs",
          className
        )}
      >
        {label}
      ...
      <div className={cn("h-full w-[2px] bg-background", className)} />
    </div>
  );
};
