...
import { v4 } from "uuid";
import { db } from "./db";
// import { updateProductTaux, validateProductionTaux } from "./utils"; // Removed import from utils

const avatarUrl = "https://cdn-icons-png.flaticon.com/512/3607/3607444.png";

import { NotificationType } from "@prisma/client";
import { cacheTag, revalidateTag, updateTag } from "next/cache";
import { formatAmount } from "./utils"; // Import formatAmount for validation messages
...
export const createProduct = async (phaseId: string) => {
  try {
    ...
    revalidateTag(`phase-product-${phaseId}`, "max");
    revalidateTag(`project-details-${phase.projectId}`, "max");
...
export const deleteProduct = async (id: string) => {
  try {
    ...
    revalidateTag(`phase-product-${product.phaseId}`, "max");
    revalidateTag(`project-details-${product.Phase.Project.id}`, "max");

    if (product.Phase.Project.unitId) {
      revalidateTag(`unit-productions-${product.Phase.Project.unitId}`, "max");
    }
...
export const createProduction = async (
  ...
  try {
    ...
    // Récupérer l'ID de l'unité et du projet pour l'invalidation du cache
    const productWithUnit = await db.product.findUnique({
      where: { id: productId },
      include: {
        Phase: {
          include: {
    ...
    if (productWithUnit?.Phase?.Project) {
      const project = productWithUnit.Phase.Project;

      // Invalidate unit-level cache
      if (project.unitId) {
        revalidateTag(`unit-productions-${project.unitId}`, "max");
      }

      // Invalidate project-level cache
      revalidateTag(`project-details-${project.id}`, "max");
    }

    revalidateTag(`productions-${productId}`, "max");
    revalidateTag(`product-${productId}`, "max");
